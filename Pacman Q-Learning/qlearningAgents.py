# python 3.6 script
#
# Reference: http://ai.berkeley.edu/reinforcement.html
#
...
class QLearnAgent(Agent):
    ...
    # Constructor, called when we start running
    def __init__(self, alpha=0.2, epsilon=0.05, gamma=0.8, numTraining = 10):
        ...
        #
        # These values are either passed from the command line or are set to the default values above. We need to create and set variables for them
...
#        print(self.epsilon)
    ...
    def getAction(self, state):
        ...
        # The data we have about the state of the game the legal action of this state
        legal = state.getLegalPacmanActions()
        if Directions.STOP in legal:
            legal.remove(Directions.STOP)
        ...
        # e-greedy - pass on the epsilon value and if epsilon is higher, go for exploration
        if util.flipCoin(self.epsilon):
            action =  random.choice(legal)
        else:
            action =  self.agentTrainingRunning(state)
